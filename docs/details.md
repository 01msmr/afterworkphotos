...
**Beside** it stands the plain list of the towns the archive knows, alphabetical, no numbers, no bullets, in the descriptions' own size (`--meta-size` — never smaller), its foot on the plan's foot and **never taller than the plan**: the names fill one column to that height and start the next (`column-fill: auto`), so a growing archive takes more columns rather than more height. It is its own element, so the map's 1.8 zoom does not reach it. A click on a town cuts the wall to its newest print and lights it; further clicks walk through that town's prints (`goToPlace()`), and the map stays up throughout — one can go from town to town without ever losing it. From the map's Tab station the **arrows walk the list** (the town under the mark is set in bold) and `Enter` or `Space` goes to it — the same move the click makes. Neither ever leaves the map: shut, they open it and mark the first town; open, they go to the marked one. Only `m`, or a click on the plan, puts it away. Prints without a place are not on the list. The choice is kept in `localStorage` (`map`); a visitor who left it up sees it unfold as the gallery opens.
