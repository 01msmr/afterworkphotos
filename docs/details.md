...
**The map — the MAP icon, grown.** There is no second map object: the small dark square at the lower left (`.mapgo`, the knobs' mirror on the `--side` line) *is* the map. Shut it is a mini town plan seen from above — streets cut out of the dark square so the wall shows through them, blocks, a turning loop, a river along the bottom — and three of those streets happen to read as **M, A and P**: a serpentine climbing in switchbacks, a triangular block with its connecting lane, a dead end ending in a turning loop. They run in the same light weight as the roads around them and carry on into the network at both ends, so the word is found rather than read.

Shown and hidden by a click on it, by Enter on its Tab station, or by the key **`m`**, it grows to **four times its size with its left and bottom edges staying put**, so it unfolds towards the top right (0.6 s). The prints shrink the same way — their top and right edges stay — because every section opens `--map-grown` plus a `--gap` of room at its left; nothing is measured, the CSS shares the geometry. On the grown square stands the plain list of the towns the archive knows, alphabetical, no numbers, no bullets, in the descriptions' small type, over a veil that keeps the plan readable underneath. A click on a town cuts the wall to its newest print and lights it; further clicks walk through that town's prints (`goToPlace()`), and the map stays up throughout — one can go from town to town without ever losing it. Prints without a place are not on the list. The choice is kept in `localStorage` (`map`); a visitor who left it up sees it unfold as the gallery opens.
