...
**The wall map — the plan one carries along.** A square like a print (sheet shadow, dims and lights like one, captioned `map`), **fixed at the left of the window** at the prints' height and, when up, their size, dark grey, with the streets of an imagined town as transparent lines — the wall shows through — and the archive's towns as dots in their true relative position (`buildMap()`: equirectangular from the `places` centre coordinates, distances from the middle compressed by a power of 0.6 so the Lake Constance cluster is not crushed by Venice and Bielefeld), sized by how many prints come from there, labelled `markdorf 147`. A click on a town cuts the wall to its newest print and lights it; further clicks walk through that town's prints (`goToPlace()`). Prints without a place are not on the map. Shown or hidden by the **MAP button** at the lower left (`.mapgo`, the knobs' mirror on the `--side` line, zoomed 1.8× like them — a dark square with MAP set askew and pressed into it, its strokes transparent lines like streets; quiet at 12 % until the mouse is on it), by Enter on its Tab station, or by the key **`m`**: the map slides in from outside the left edge (`html.map-on`) while every section opens the same room at its left — `--map-w`, one of (prints + 1) equal squares between the `--side` margins, `--cols` following `data-per-row` on `<html>` — so the prints on screen scale down to three equal squares in one 0.6 s motion; nothing is measured, the CSS shares the geometry (`--gap`, `--wall-top`). Being fixed, the map stays through every glide and cut: one can go from town to town without ever losing it. The choice is kept in `localStorage` (`map`); a visitor who left it up sees it slide in as the gallery opens.
