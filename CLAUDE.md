...
- **`http://127.0.0.1:8765/tests/gallery.html`** — the gallery on the bench: boot without errors, every module served, the loop's steps, the console's layout, a ride (frames, pictures on the GPU), the pointer with two made-up hands, the options sheet (pinned clear of frames, a press sets a line without a re-hang, the tape takes it down), a doubled label's shadow. Every line ok before a gallery change is pushed; add a line when a behaviour changes. The window must be on screen.
