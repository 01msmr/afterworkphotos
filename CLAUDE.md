...
- **The gallery's modules get their version from the import map in `gallery/index.html`** (since 2026-09-19): the modules import bare names (`from 'gallery/frames'`), the map turns them into `/res/gallery/frames.js?v=…`. Bump that one file — `sed -i '' 's/?v=OLD/?v=NEW/g' gallery/index.html` — and never write a `?v=` into a module. A new module needs a line in the map.
